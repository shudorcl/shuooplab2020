...
public:
    ...
    bool NameDelete(string name) throw(int);
    bool IDDelete(int the_id) throw(int);
    int FindId(string name);
    string FindName(int id);
    void ChangeId(string name, int id, int correct_id);
    void ChangeName(string name, int id, string correct_name);
...
template <typename T>
Node<T> *LinkList<T>::CurNode()					// 返回(当前结点的地址（NUL)L表示无当前结点）
{
	return cur_node;
}
...
template <typename T> template <typename TYPE>
Node<T> *LinkList<T>::Locate_name(const TYPE &x, bool newsearch) // 当前结点依条件（给定TYPE类型）定位
{
	static Node<T> *p=head;
	static TYPE data = x;
	if(newsearch)
    ...
    for(; p!=NULL && p->data.name!=data; p=p->next)	// for循环的表达式1为空：从链表的"当前"结点起继续查找
	 	;													// TYPE()为类型强制转换运算符
	cur_node = p;
	if(p!=NULL)		// 如果找到，在返回找到的结点地址（cur_node）之前，p"悄悄"地向前走一步，为继续查找做好准备
		p = p->next;
...
template <typename T> template <typename TYPE>
Node<T> *LinkList<T>::Locate_id(const TYPE &x, bool newsearch) // 当前结点依条件（给定TYPE类型）定位
{
    static Node<T> *p=head;
    static TYPE data = x;
    if(newsearch)
    ...
    for(; p!=NULL && p->data.ID!=x; p=p->next)    // for循环的表达式1为空：从链表的"当前"结点起继续查找
         ;                                                    // TYPE()为类型强制转换运算符
    cur_node = p;
    if(p!=NULL)        // 如果找到，在返回找到的结点地址（cur_node）之前，p"悄悄"地向前走一步，为继续查找做好准备
        p = p->next;
...
template <typename T> template <typename TYPE>
string LinkList<T>::Locate_id2(const TYPE &x, bool newsearch) // 当前结点依条件（给定TYPE类型）定位
{
    static Node<T> *p=head;
    static TYPE data = x;
    if(newsearch)
...
template <typename  T>
bool  LinkList<T>::Duplicate_name(string name) {
    cur_node = head;
    while(cur_node->data.name != name)
        cur_node=cur_node->next;
    if (cur_node == NULL)return false;
    return true;
}

template <typename T> bool  LinkList<T>::Duplicate_id(int id) {
    cur_node = head;
    while(cur_node->data.ID != id)cur_node=cur_node->next;
    if (cur_node == NULL)return false;
    return true;
}

template <typename T>
bool LinkList<T>::NameDelete(string name) throw(int) {
    cur_node = head;
    while(cur_node->data.name != name)cur_node=cur_node->next;
    this->DeleteCurNode();
    return 1;
}

template <typename T>
bool LinkList<T>::IDDelete(int the_id) throw(int) {
    cur_node = head;
    while(cur_node->data.ID != the_id)cur_node=cur_node->next;
    this->DeleteCurNode();
    return 1;
}
template <typename T>
int LinkList<T>::FindId(string name){
    cur_node = head;
    while(cur_node->data.name != name)cur_node=cur_node->next;
...
template <typename T>
string LinkList<T>::FindName(int id) {
    cur_node = head;
    while(cur_node->data.ID != id)cur_node=cur_node->next;
    if (cur_node == NULL)return "Find nothing";
    return cur_node->data.name;
}

template <typename T>
void LinkList<T>::ChangeId(string name, int id, int correct_id) {
    cur_node = head;
    while(1){
        if(cur_node->data.name == name){
            if(cur_node->data.ID == id)break;
        }
        cur_node=cur_node->next;
    }
    if (cur_node==NULL)
        cout<<"find no book"<<endl;
    else
        cur_node->data.ID = correct_id;
}

template <typename T>
void LinkList<T>::ChangeName(string name, int id, string correct_name) {
    cur_node = head;
    while(1){
        if(cur_node->data.name == name){
            if(cur_node->data.ID == id)break;
    ...
    if (cur_node==NULL)
        ...
        cur_node->data.name = correct_name;
}
#endif
